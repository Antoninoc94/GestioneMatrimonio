...
import { Plus, Pencil, Trash2, CheckCircle, Circle, AlertCircle } from 'lucide-react';
import api from '../api';
...
export default function Scadenze() {
  ...
  const completate = items.filter(i => i.completata);
  ...
  const ScadenzaCard = ({ s }) => {
    ...
    const isOggi = s.data_scadenza && isToday(parseISO(s.data_scadenza));
    return (
      <div className={`flex items-start gap-3 p-3 rounded-lg border ${s.completata ? 'bg-gray-50 border-gray-100 opacity-60' : isScaduta ? 'bg-red-50 border-red-200' : isOggi ? 'bg-yellow-50 border-yellow-200' : 'bg-white border-gray-100'}`}>
        <button onClick={() => toggle(s)} className="mt-0.5 flex-shrink-0">
          {s.completata ? <CheckCircle size={20} className="text-green-500" /> : <Circle size={20} className="text-gray-300 hover:text-rose-400" />}
        </button>
  ...
  const Section = ({ title, items, empty: emptyMsg }) => items.length === 0 ? null : (
    <div className="mb-6">
    ...
    <div>
      <div className="page-header flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="page-subtitle">{pendenti.length} da fare · {completate.length} completate</p>
        </div>
        <button className="btn-primary" onClick={openNew}><Plus size={16} /> Nuova Scadenza</button>
      </div>

      <Section title="Scadute" items={scadute} />
      <Section title="Oggi" items={oggi} />
      <Section title="Da fare" items={future} />

      {pendenti.length === 0 && !mostraCompletate && (
        <div className="text-center py-12 text-gray-400 card">
          <CheckCircle size={40} className="mx-auto mb-2 text-green-300" />
          <p className="font-medium">Tutto fatto! Nessuna scadenza pendente.</p>
        </div>
      )}
      ...
      {completate.length > 0 && (
        <div>
          ...
          {mostraCompletate && <div className="space-y-2">{completate.map(s => <ScadenzaCard key={s.id} s={s} />)}</div>}
        </div>
      )}
